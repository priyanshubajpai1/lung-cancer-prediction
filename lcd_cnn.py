    ...
    def preprocess_data(self):
        ...
        for num, patient in enumerate(self.lungPatients):
            ...
            try:
                ...
                print(f'Patient {patient} is unlabeled or has an error.')

        # Save all lists into a single compressed .npz file
        np.savez('imageData.npz', images=images, labels=labels, patients=patients)
        # --- END OF CHANGES ---
...
# Data training is the process of training the model based on the dataset and then predict on new data.
    def train_data(self):
        ...
        # --- START OF CHANGES ---
        # Load the .npz file
        data = np.load('imageData.npz', allow_pickle=True)
        
        # Extract the arrays by their keys
        images = data['images']
        labels = data['labels']
        ...
        val_X = images[45:50]
        val_y = labels[45:50]
        val_patients = patients_data[45:50]
        # --- END OF CHANGES ---

        training_data=Label(text="Total Training Data: " + str(len(train_X)),font=("Times New Roman",13,"bold"),bg="black", fg="white",)
        training_data.place(x=750,y=150,width=200,height=18)

        validation_data=Label(text="Total Validation Data: " + str(len(val_X)),font=("Times New Roman",13,"bold"),bg="black",fg="white",)
        ...
        # ... (Keep the cnn, convolution3d, and maxpooling3d functions exactly the same) ...
        ...
        def cnn(x):
            ...
            convolution5 = maxpooling3d(convolution4) # Typo in original code, should be convolution5
            ...
            with tf.Session() as session:
                ...
                for epoch in range(epochs):
                    ...
                    for X, Y in zip(train_X, train_y):
                        try:
                            _, c = session.run([optimizer, cost], feed_dict={x: X.reshape(-1, size, size, NoSlices), y: Y.reshape(-1, 2)})
                            epoch_loss += c
                ...
                # --- CLEANER EVALUATION ---
                val_accuracy = accuracy.eval({x: val_X, y: val_y})
                print('Epoch', epoch + 1, 'completed out of', epochs, 'loss:', epoch_loss)
                print('Accuracy:', val_accuracy)
                
                final_accuracy=Label(text="Final Accuracy: " + str(val_accuracy),font=("Times New Roman",13,"bold"),bg="black", fg="white",)
                final_accuracy.place(x=750,y=230,width=200,height=18)

                # ... (The rest of the result display and confusion matrix logic can be simplified as well) ...
                
                # Simplified result processing
                final_preds = session.run(tf.argmax(prediction, 1), feed_dict={x: val_X})
                actual_preds = session.run(tf.argmax(y, 1), feed_dict={y: val_y})

                patients = val_patients
                actual = ["Cancer" if i == 1 else "No Cancer" for i in actual_preds]
                predicted = ["Cancer" if i == 1 else "No Cancer" for i in final_preds]
                ...
                y_actual = pd.Series(actual_preds, name='Actual')
                y_predicted = pd.Series(final_preds, name='Predicted')

                df_confusion = pd.crosstab(y_actual, y_predicted).reindex(columns=[0,1],index=[0,1], fill_value=0)
                print('Confusion Matrix:\n')
                print(df_confusion)

                # ... (Rest of your Tkinter GUI update code remains the same) ...
